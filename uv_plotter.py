...
def generate(excited_states, plt_range, grid_size, sigma = 0.4, normalization=True):
    ...
    cnst = math.sqrt(math.pi) * electron_charge**2 * constants.Avogadro / ( 1000 * math.log(10) * constants.c**2 * electron_mass ) 
    cnst = 1.3062974e8

    sigma = np.power(10, 9) * constants.Planck * constants.c / 1.602176634e-19 / sigma # convert to wavenumber

    for point in grid:
        fit = 0.0
        for (wv, os) in excited_states:
            fit += cnst * os / sigma * np.exp( - np.power((10**7 / point - 10**7 / wv) / (sigma), 2))
